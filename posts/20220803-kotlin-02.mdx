...
- 일반적인 서브루틴은 오직 한 가지 진입 지점만을 가진다. 함수를 호출하는 부분이며, 그때마다 활성 레코드 (activation record)가 스택에 할당되면서 서브루틴 내부의 로컬 변수등이 초기화 된다. 또한 서브루틴에서 반환되고 나면 활성 레코드가 스택에서 사라지기 때문에 모든 상태를 잃어버린다. 
...
- runBlocking : coroutine builder 로서 내부 코루틴이 모두 끝난 다음에 반환된다.
- launch : coroutine builder로서 , 넘겨받은 코드 블록으로 새로운 코루틴을 생성하고 실행시켜준다.
- yield : 해당 코루틴이 실행권을 양보하고, 실행 위치를 기억하고, 다음 호출때는 해당 위치부터 다시 실행한다.

위 코루틴에서 1,3,5를 출력하는 코루틴과 2,4,6를 출력하는 코루틴이 서로 실행권을 양보해가면서 실행된다. 한가지 유의할점은 마치 병렬적으로 실행되는 것처럼 보이지만 다른 쓰레드가 아니라 하나의 쓰레드에서 수행된다는 점이다. 따라서 Context Switching 도 발생하지 않는다.  

- Launch coroutine Builder는 Job 객체를 반환한다. Job은 N개 이상의 coroutines의 동작을 제어할 수도 있으며, 하나의 coroutines 동작을 제어할 수도 있다. 
...
- join : job을 실행하고, Job의 동작이 완료될 때까지 job을 호출한 코루틴을 일시중단한다. 
...
정리하면 async는 코드 블록을 비동기로 실행 할 수 있고, async가 반환하는 Deffered의 await를 사용해서 코루틴이 결과 값을 내놓을 때까지 기다렸다가 결과값을 받아올 수 있다. 

이때 비동기로 실행할 때 제공되는 코루틴 컨텍스트에 따라 하나의 Thread안에서 제어만 왔다 갔다 할수도 있고, 여러 Thread를 사용할 수도 있다. 
...
after d2
after d3
1+2+3 = 6 // 코루틴이 결과값을 내놓을 때까지 기다렸다가 결과값을 받아온다. 
```
...
- delay , yield 함수는 일시중단 함수로 이외에도 다른 일시중단 함수들이 존재한다.
  ...
  - joinAll : 모든 작업이 종료될 때까지 현재 작업 대기    
