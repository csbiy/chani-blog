...
- 네트워크에서는 통신할 때의 규약을 protocol이라고 함. 
...
네트워크 인터페이스 카드에 들어오는 전기 신호를 데이터 형태(bit열)로 만들고 bit열에서 도착지 MAC 주소를 확인한다. 이때 자신의 MAC 주소와 도착지 MAC 주소가 동일하면 처리하고, 그렇지 않으면 폐기한다. 
...
이 MAC 주소 테이블을 보고 단말들간에 통신할 때 정확한 MAC 주소가 위치하는 포트로 포워딩해준다. 
...
상위 protocol 지시자가 필요한 이유는 decapsulation하는 목적지 쪽에서 헤더에 아무 정보가 없으면 어떤 상위 protocol로 올려주어야 할지 결정할 수 없기 때문이다. 
