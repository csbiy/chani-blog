...
- 일반 SQL문에서는 순서와 타입으로만 매핑된다. 즉 이름은 가독성에만 영향을 끼치고 실제 실행은 USING절에 오는 순서와, 타입 기반으로 실행된다. ( 프로시저로 매핑할 때는 변수 이름까지 맞춰주어야 한다. )
...
```EXECUTE IMMEDIATE 동적쿼리문 BULK COLLECT INTO 중첩테이블변수 USING bind변수``` 형태로 사용이 가능하다.
