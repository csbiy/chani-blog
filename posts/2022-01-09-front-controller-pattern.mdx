...
front controller pattern은 front controller와 요청을 위임할 class (controller) 로 구성된다. 이때 요청을 위임할 class는 공통 abstract class 또는 interface를 상속하고 있다.
...
```
실제 DispatcherServlet 내부구현은 훨씬 복잡한데, 이중에서 adapter가 ModelAndView 객체를 반환해주고, 
