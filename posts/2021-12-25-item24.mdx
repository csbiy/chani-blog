...
정적 멤버 class는 흔히 바깥 class와 함께 쓰일 때만 유용한 public 도우미 class로 쓰인다. 
...
private 정적 멤버 class는 바깥 class가 표현하는 객체의 한 구성요소를 나타낼 때 사용한다.  ex) Map 구현체의 Entry class
...
멤버 class instance와 바깥 class instance간에 관계는 멤버 class가 instance화될 때 확립되며, 더이상 변경될 수 없다. 이 관계는 보통 바깥 class의 method에서 멤버 class의 생성자를 호출할 때 자동으로 만들어진다. 
...
(Q: 어댑터를 만들 때 내부 멤버 class 를 사용하는 지)
...
비정적인 문맥에서 사용될 때만 바깥 class의 instance를 참조할 수 있고, 정적 문맥에서라도 상수 변수 이외의 정적 멤버는 가질 수 없다. 
...
익명 class 처럼 비정적 문맥에서 사용될 때만 바깥 instance를 참조할 수 있으며, 정적 멤버는 가질 수 없으며 가독성을 위해 짧게 작성되야 한다. 
