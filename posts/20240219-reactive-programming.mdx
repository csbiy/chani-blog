...
3. Resilient (회복성)
- 시스템에 장애가 발생하더라도 응답성을 유지하는 것을 의미한다. 시스템의 구성요소들이 독립적으로 분리되기 때문에 장애가 발생하더라도 전체 시스템은 여전히 응답 가능하고, 장애가 난 부분만 복구하면 된다는 의미입니다.
...
- 데이터 소스의 변경이 있을 때 마다 데이터를 전파하는 이벤트 기반 아키텍쳐를 사용합니다. 
...
public interface Subscriber<T> {
    ...
    public void onComplete(); // 데이터 통지를 완료했을 때 호출되는 메소드 (후처리)
}
```
3. Subscription 
...
## Project Reactor 
- Spring Framework 5버전부터 Reactive Stack에 포함되어 Spring WebFlux 기반의 리액티브 application 을 개발할 때 핵심 역할을 담당하고 있습니다. 
