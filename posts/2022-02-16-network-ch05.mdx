...
2. 브로드캐스트 컨트롤  
- 패킷의 목적지 주소가 Routing table에 없는 경우 패킷을 버린다. 
- 멀티캐스트 정보를 습득하지 않고 브로드캐스트 패킷을 전달하지 않는다. (브로드캐스트가 다른 네트워크로 전파되는것을 막는다. )

3. 프로토콜 변환 
- 패킷 포워딩 과정에서 2계층 헤더를 변경할 수 있다.
- 3계층에서 동작하는 장비이기 때문에 2계층 헤더를 벗겨내고, 다시 2계층 헤더 정보를 새로 만들어 외부로 내보내는데, 이때 헤더 정보를 변경할 수 있다. 
...
- IP주소와 Subnet mask로 해당 IP주소의 네트워크 주소 정보를 알 수 있다. 이 정보로 해당 네트워크에 대한 라우팅 테이블을 자동으로 만드는데, 이때 이 경로 정보를 Direct Connected라고 부른다. 
...
( *SPoF(Single Point Of Failure, 단일 장애점) : 한 요소에서 장애발생시, 전체 시스템에서 장애가 발생하는 지점 )
