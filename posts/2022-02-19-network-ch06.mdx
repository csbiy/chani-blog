...
- 4계층 장비(세션 장비)는 통신의 방향성이나 순서와 같은 통신 전반에 관한 관리가 필요한데, 이러한 정보를 <strong>세션 테이블</strong>이라는 공간에 담아 관리한다.   
...
- 세션 테이블을 사용하는 경우  : 패킷이 외부로 나갈 때 세션 정보를 세션 테이블에 저장하고, 패킷이 들어올때 내부 사용자에 대한 응답인지, 외부에서 처음 온 요청인지 판단 
...
- 비대칭 경로 : 인바운드 패킷과 아웃바운드 패킷이 다른 세션 장비를 통과
...
- 세션장비에서 두 세션 중 한 세션만 끊긴 경우 통신을 하지 못할 수도 있음. 
...
2. FTP서버는 데이터 송신용 포트번호를 client에게 알림
