...
예를 들면 아래의 코드는 매번 실행될 때마다 JVM 메모리의 Heap 영역에 객체가 저장되고, 그 객체를 가리키는 참조값인 newObj가 반환된다. 
```java
...
static method를 통해 instance 생성 이전에 class loading 시점에서 memory 공간이 할당되고 초기화되므로, 불필요한 객체 생성을 피할 수 있다.
