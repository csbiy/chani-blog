...
tags: ["Nodejs", "express"]
---
middleware 는 request, response 의 중간에 위치하며, request,response에 필요한 작업을 수행한다. router,error handler 모두 middleware의 일종이다. 
...
보통 모든 middleware에서 요청 url을 찾을 수 없을 때를 대비하기 위해, 가장 아래 middleware에 error handling 용 middleware를 두어 처리한다.
```javascript
