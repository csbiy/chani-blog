...
## Partitioner 종류
UniformStickyPartitioner , RoundRobinPartitioner가 있는데 둘 다 KEY가 존재하는 경우에는 KEY의 해시값을 통해 partition을 매칭시킨다. KEY가 존재하지 않을 때 동작이 다르다.
- UniformStickyPartitioner : Accumulator에서 데이터가 배치로 모두 묶일 때까지 대기하였다가, 모두 동일한 파티션에 전송
...
특정 데이터를 특정 파티션으로 보내고 싶을 때는 Partitioner를 구현해서 등록하면 된다. 
