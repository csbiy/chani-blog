...
외부에서 멤버필드 값을 조회,변경할 때는 getter , setter 방식을 사용한다. 
...
예를 들어 기존에 client가 필드를 직접 참조해서 사용하였을 때는 Point class의 내부 표현 방식을 바꾸기 어려울 것이다. 
...
default,private class에서는 필드를 노출하는 편이 나을 때도 있다.
