...
```
<figure>
  <img src="/images/BATCH_STEP_17.png"></img>
...
<figure>
  <img src="/images/BATCH_STEP_16.png"></img>
...
<figure>
  <img src="/images/BATCH_STEP_13.png"></img>
...
```
<figure>
  <img src="/images/BATCH_STEP_14.png"></img>
        <figcaption>BATCH_JOB_EXECUTION_CONTEXT 테이블</figcaption>
</figure>

<figure>
  <img src="/images/BATCH_STEP_15.png"></img>
...
<figure>
  <img src="/images/BATCH_STEP_01.png"></img>
...
언제까지 반복해서 실행할것인가에 대한 판단은 Tasklet 객체에서 반환하는 ```RepeatStatus```값에 의해 결정됩니다. ```RepeatStatus.FINISHED``` 와 같이 특정 ```RepeatStatus```를 반환할 때까지 계속해서 실행합니다.
...
<figure>
  <img src="/images/BATCH_STEP_02.png"></img>
        <figcaption>Spring Batch에서 제공하는 Step 구현체 종류</figcaption>
</figure>
...
- step 실패시 step이 재수행될 수 있는데, 이때 재수행횟수를 제한하는 API입니다. 
...
### 설정 예시
- 아래 테스트 케이스는 startLimit을 3으로 주었을 때입니다.
```java
public Step step(){
...
<figure>
  <img src="/images/BATCH_STEP_03.png"></img>
...
<figure>
  <img src="/images/BATCH_STEP_04.png"></img>
        <figcaption>startLimit 3으로 설정하고 STEP 재시작시 BATCH_JOB_EXECUTION 테이블 상태</figcaption>
</figure>

### allowStartIfComplete API

- step 실패시 step이 재시작될 수 있는데, 이때 기존에 완료된 step은 재시작할지 말지를 결정하는 api입니다.
- 기본값은 false로 완료된 step은 재시작을 수행하지 않습니다. 만약 true로 주게 된다면 이미 ```COMPLETE```된 step이더라도 항상 재시작합니다. 


### 설정 예시
- 아래 테스트 케이스는 ```Job```을 첫 번째 step은 ```allowStartIfComplete```값을 ```true```로 준 step이고, 두번째 step은 항상 실패하는 step입니다. 
   즉 두 번째 step에서 항상 실패함으로 step1은 ```allowStartIfComplete```값이 ```true```라면 같이 재시작됩니다.
```java
@Bean
public Job batchJob() {
...
- 5번 Job을 재시작해보면 ```allowStartIfComplete```을 ```true```로 준 첫 번째 step은 이미 step이 완료되었음에도 불구하고 항상 재시작 되는 것을 확인할 수 있습니다.
- 항상 실패하는 두 번째 step은 ```startLimit``` 값을 초과하게 되면 재시작되지 않고, 첫 번째 step만 계속해서 재시작됩니다.
<figure>
  <img src="/images/BATCH_STEP_05.png"></img>
...
- ```DefaultJobParametersExtractor```는 부모 Job의 parameter와 Step의 parameter를 가져올 수 있습니다. 이때 가져올 parameter의 key값을 설정해줄 수 있습니다. 
...
<figure>
  <img src="/images/BATCH_STEP_06.png"></img>
...
<figure>
  <img src="/images/BATCH_STEP_07.png"></img>
        <figcaption> JobStep 구성한 Job의 BATCH_JOB_EXECUTION 테이블 상태</figcaption>
</figure>

<figure>
  <img src="/images/BATCH_STEP_08.png"></img>
...
<figure>
  <img src="/images/BATCH_STEP_09.png"></img>
        <figcaption> JobStep 구성한 Job의 BATCH_JOB_EXECUTION 테이블 상태</figcaption>
</figure>
...
<figure>
  <img src="/images/BATCH_STEP_10.png"></img>
        <figcaption> JobStep 구성한 Job의 BATCH_STEP_EXECUTION 테이블 상태</figcaption>
</figure>
...
<figure>
  <img src="/images/BATCH_STEP_11.png"></img>
        <figcaption> JobStep 구성한 Job의 BATCH_JOB_EXECUTION 테이블 상태</figcaption>
</figure>


<figure>
  <img src="/images/BATCH_STEP_12.png"></img>
        <figcaption> JobStep 구성한 Job의 BATCH_STEP_EXECUTION 테이블 상태</figcaption>
</figure>
