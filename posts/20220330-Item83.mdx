...
- 만약 성능때문에 정적필드를 지연 초기화해야 한다면 lazy initialization holder class 관용구를 사용하자 
...
위처럼 전용 Holder class를 만들어두면 getField()method가 호출될 때 FieldHolder 클래스 초기화가 된다. 이렇게 사용하게 되면 클래스 초기화가 최초에 끝난 뒤에는 아무런 검사나 동기화 없이 필드에 접근하게 된다. 
...
- 성능 때문에 멤버필드를 지연 초기화해야 한다면 double check 관용구를 사용하자 

이 방식은 필드의 값을 두 번 검사하는 방식으로 , 최초는 동기화 없이 검사하고 두 번째는 동기화하여 검사한다. 두번째 검사에서도 필드가 초기화되지 않았을 때만 필드를 초기화한다. 필드가 초기화된 이후에는 동기화하지 않으므로 해당 필드를 반드시 volatile(메인메모리로부터 저장하고 읽어옴) 로 선언해야 한다. 
...
```
 ...
 - 가끔 반복해서 초기화해도 상관없는 멤버필드를 지연 초기화할 때가 있는데, 이럴 때에는 이중검사에서 단일검사로 (Single Check) 관용구로 변경해도 된다.
