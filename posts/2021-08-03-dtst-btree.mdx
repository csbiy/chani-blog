...
( 균형 트리란 ? 왼쪽,오른쪽 subtree의 높이 차이를 줄여, O(logN)의 시간복잡도를 가지고, 탐색,삽입,삭제 연산을 수행할수 있다. )

대표적으로 AVL Tree, RB Tree, B tree...등이 있다. 

차수가 M인 B트리는 다음과 같은 정의를 갖는다. 
(= 트리 노드의 최대 자식 노드의 개수가 M이며, 이때 M은 2이상의 정수여야한다.)
...
탐색과정을 거쳐, 새로운 키가 저장되야할 leef node를 찾고, 만약에 leef node에 새 키를 수용할 공간이 있으면, 정렬상태를 유지하도록 key를 삽입한다. 근데 만약에 M-1개의 키를 이미 가지고 있다면, 하나의 키가 추가되면 총 M개의 키에 차수의 개수는 M+1이 될 것이다.
...
위에 차수가 3인 B Tree에 45를 삽입하면 (25,35) Node에서 (25,35,45) 의 노드 키의 중간 키인 "35"를 부모노드로 올리고, 나머지인 25,45는 각각의 노드에 저장된다. 
...
위와 같이 차수가 3인 B tree가 있다고 가정할 때, 
