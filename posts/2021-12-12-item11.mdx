...
- Java HashMap은 hash함수 적용할 때, hashCode값을 사용함으로 다른 두 객체가 hashCode값이 같다면 두 객체는 같은 인덱스로 해쉬됨(collision)으로 성능상 단점이 있음. 
...
1. int 변수 result를 선언한 후 객체의 첫 번째 핵심필드 (equals에서 값비교시 사용하는 첫필드)의 해시 코드 계산해 초기화 
...
public class PhoneNumber {
   ...
   // ...equals method 생략
    @Override
    public int hashCode() {
       // hashCode 가 실제로 호출될 때 값이 default면 초기화 
        int result = hashCode;
        if(result == 0){
...
- 논리적으로 다른 객체인 경우, 같은 hashCode값을 가질수도 있으나, hashMap등에서 사용할 때 collision이 발생해 최악의 경우 O(n) 시간복잡도를 가지게 성능을 떨어트린다.
