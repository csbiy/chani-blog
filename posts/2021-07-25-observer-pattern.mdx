...
- 상태가 계속 변하는 subject 객체에 interface를 두어, observer를 등록, 해제, observer에게 상태변화를 알리는 공통적인 method를 둔다.
- subject 객체의 상태를 알림받는 observer 객체에서 interface를 두어, observer의 변화시, 변화에 따른 logic을 처리하는 method를 둔다.
- subject, observer는 인터페이스 추상화 계층을 통해서 coupling되어 있다. 즉, 하위 구현 class가 어떻게 구현되었는지는 관심도 없고 알 필요도 없다. (=loose coupling)
...
어쨌거나 위의 기상 시스템 app을 예시로 observer pattern을 살펴보면 다음과 같다. 
...
public class TestWeatherStation {
    ...
    // --------output--------
    // Current Conditions : 80.0/65.0/30.4
    // Current Conditions : 40.0/25.0/20.4
    // Current Conditions : 60.0/75.0/29.4
}
