...
client에게 compiler 경고가 나오는데 Class.cast method는 타입 안전함을 확인하고 비검사 경고를 제거해주기 때문이다. 
...
이를 해결하는 방안으로 값을 넣을 때에도 매번 동적 형변환하는 방법이 있다.
...
그 이유는 Generic간의 class는 같은 Class 객체를 공유하기 때문이다. 예를 들어 List\<String>.class와 List\<Integer>.class 는 List.class라는 같은 Class객체를 공유한다. 
...
container (일급 collection)  자체를 generic class로 만드는 것보다 key 값만 generic으로 변경한 타입 안전 이종 컨테이너는 타입 제약에서 비교적 자유로우며, Class를 key 로 써서 값을 넣고 뺀다, 이때 parmeter로 넘어오는 class literal을 type token이라고도 부른다.
