...
2. 태그 값마다 필요한 필드가 다른데, 이를 모두 하나의 class에 보관함으로, 객체를 생성하였을 때 사용되지 않는 필드 (메모리 낭비) 도 같이 초기화 된다.
...
- 태그 달린 class는 위와 같은 단점들을 가지고 있기 때문에, 클래스 계층 구조로 변경하는게 더 바람직하다. 
- 변경 방법을 정리하면 다음과 같다.
