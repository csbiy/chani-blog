---
id: 185
title: "OpenTelemetry + Grafana Stack (Tempo, Mimir, Loki) - 통합 관측성 시스템 구축"
description: "OpenTelemetry로 Traces, Metrics, Logs를 수집하고 Grafana Stack(Tempo, Mimir, Loki)으로 통합 관측성을 구축하는 방법"
date: "2026-01-25 14:30:00"
tags: ["OpenTelemetry", "Grafana", "Tempo", "Mimir", "Loki", "Observability", "Monitoring", "Tracing"]
---
...
**통합 관측성(Unified Observability)**은 Traces, Metrics, Logs 세 가지를 하나의 플랫폼에서 통합해서 보는 것이다. 하나의 대시보드에서 메트릭 이상을 발견하면 클릭 한 번으로 해당 시점의 로그와 트레이스로 바로 이동할 수 있다.

```text
기존 방식: Metrics(Prometheus) → Logs(Kibana) → Traces(Jaeger) → 각각 별도 조회
통합 관측성: Grafana 하나에서 Metrics → Logs → Traces 연결
```
...
HTTP 요청을 보낼 때 `traceparent` 헤더에 Trace Context를 담아서 전송한다.
...
│   │           └─ 16자리 Parent Span ID
│   └─ 32자리 Trace ID
└─ 버전 (00 = W3C Trace Context v1)
```

수신하는 서비스는 이 헤더를 읽어서:
1. 동일한 Trace ID를 사용
2. 새로운 Span ID를 생성
3. 받은 Span ID를 Parent Span ID로 설정
...
Grafana Stack은 Grafana Labs에서 만든 오픈소스 관측성 플랫폼이다. 핵심 구성 요소는 다음과 같다.
...
**출력 예시**:
```
2026-01-25 15:30:45.123 [http-nio-8080-exec-1] INFO  c.e.OrderController - [traceId=5f3a4b2c1d8e9f7a, spanId=3c9d1e4f] - Creating order for user: 12345
```
...
| 배포 방식 | 장점 | 단점 | 추천 상황 |
|----------|------|------|----------|
| **Sidecar** | Pod별 격리, 설정 유연 | 리소스 중복 | 팀별 독립 운영 |
| **DaemonSet** | 리소스 효율적, 중앙 관리 | Node 장애 시 영향 큼 | 단일 플랫폼 팀 운영 |

일반적으로는 **DaemonSet**을 추천한다. 각 노드에 Collector 하나씩 띄우고, 모든 Pod이 localhost:4317로 전송하는 방식이다.
...
### Q5: Exemplar가 자동으로 추가된다는데, 어떻게 동작하나?

OpenTelemetry SDK가 메트릭을 기록할 때, 현재 활성화된 Span의 Trace ID를 자동으로 Exemplar로 첨부한다. Prometheus가 이 Exemplar를 수집하고, Grafana가 시각화할 때 Trace 링크를 생성한다.

단, Prometheus exporter에서 Exemplar를 활성화해야 한다 (기본값은 비활성화).
...
scrape_configs:
  - job_name: 'spring-app'
    scrape_interval: 15s
    # Exemplar 수집 활성화 (Prometheus 2.26+)
...
### Q6: Mimir Ingester가 죽으면 버퍼 데이터는 소실되나?
...
### Q1: Trace가 Tempo에 표시되지 않는다

**원인**: OTLP Endpoint 설정 오류 또는 네트워크 문제
...
# Tempo로 데이터가 들어오는지 확인
curl http://localhost:3200/ready
```

애플리케이션에서 직접 Tempo로 전송하는지 확인한다.

```bash
# 애플리케이션 로그에서 OTLP 전송 확인
java -javaagent:opentelemetry-javaagent.jar \
     -Dotel.exporter.otlp.endpoint=http://localhost:4317 \
     -Dotel.traces.exporter=logging \  # 로깅 추가
     -jar app.jar
```

### Q2: Logs에서 Trace로 점프가 안 된다

**원인**: 로그에 `trace_id`가 포함되지 않음

**해결**: Logback 설정에서 `%X{trace_id}` 추가 확인. OpenTelemetry Java Agent를 사용하면 자동으로 MDC에 추가되는데, 로그 패턴에 포함시켜야 한다.

```xml
<pattern>%d [%X{trace_id}] %msg%n</pattern>
```

Grafana 데이터소스 설정에서 "Derived fields" 추가:
...
### Q3: Mimir에 메트릭이 안 들어온다

**원인**: Prometheus remote_write 설정 누락

**해결**: Collector 설정에서 exporter 확인

```yaml
exporters:
  prometheusremotewrite:
    endpoint: http://mimir:9009/api/v1/push
    # Mimir가 multi-tenancy 모드면 헤더 추가 필요
    headers:
      X-Scope-OrgID: "demo"
```
...
통합 관측성 스택의 리소스 사용량을 기존 도구와 비교하면 다음과 같다.

| 구성 | 메모리 사용량 | 디스크 I/O | 비용 (월간 예상) |
|------|-------------|-----------|---------------|
| Jaeger + Prometheus + ELK | ~16GB | 높음 | ~$500 (EC2 + EBS) |
| Tempo + Mimir + Loki (Grafana Stack) | ~8GB | 낮음 | ~$150 (EC2 + S3) |

Grafana Stack이 Object Storage를 사용하므로 디스크 I/O가 낮고, 전체 비용이 약 70% 절감된다.
