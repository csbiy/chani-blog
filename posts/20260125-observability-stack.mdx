...
```text
DaemonSet (기본)
  → 모든 서비스의 기본 메트릭/로그/트레이스 수집

Sidecar (특수 케이스)
  → 결제 서비스: PCI-DSS 준수를 위한 민감 정보 필터링
  → ML 서비스: 고빈도 메트릭 수집 (초당 수만 건)
```

**비용 비교 (예시)**:

| 환경 | Sidecar 비용 | DaemonSet 비용 | 절감률 |
|------|------------|--------------|-------|
| Node 10개, Pod 100개 | Collector 100개<br/>12.8GB 메모리 | Collector 10개<br/>2.56GB 메모리 | **80% 절감** |
| Node 50개, Pod 500개 | Collector 500개<br/>64GB 메모리 | Collector 50개<br/>12.8GB 메모리 | **80% 절감** |

일반적으로는 **DaemonSet**을 추천한다. 각 노드에 Collector 하나씩 띄우고, 모든 Pod이 `$(NODE_IP):4317`로 전송하는 방식이다. AWS EKS, GKE 등 대부분의 관리형 Kubernetes에서 기본 패턴으로 사용된다.
